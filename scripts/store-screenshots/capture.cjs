...
const CONTEXT_LOCALE = LOCALE === "en" ? "en-US" : "it-IT";
...
(async () => {
  ...
  // Logged-out: onboarding hero (the auth gate only renders it without a session).
  {
    ...
    await page.waitForFunction(() => /Benvenut|Welcome/.test(document.body.innerText), null, { timeout: 240000 });
  ...
  await tap(page, LOCALE === "en" ? "Sign in" : "Accedi");
