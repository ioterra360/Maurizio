...
> Memika is being built alongside an earlier, near-finished Expo+Supabase app
...
- **Stack split**: TLC uses 5 Zustand stores by domain (auth, data, admin,
  logistics, notifications). Memika starts smaller — currently just
...
- **Styling**: TLC uses React Native `StyleSheet`. Memika uses NativeWind
...
- **Role-specific groups** beyond `(app)` and `(admin)`. TLC has
  `(client)` `(admin)` `(logistics)` with deep role logic. Memika is
  user / admin only.
- **JS over TS**. TLC is `.jsx`/`.js`. Memika is `.tsx`/`.ts` strict from
...
- **Inline-styled components**. TLC has some heavyweight cards built with
  StyleSheet + inline overrides. Memika uses NativeWind classes for
  static styling per AGENTS.md.

## What Memika does that TLC doesn't (yet)
...
- **Centralized auth-gate hook** (`useAuthGate(surface)`). TLC duplicates
  the redirect logic across multiple `_layout` files. Memika encodes it
  once.
- **Server-side admin allowlist** (Supabase `admin_emails` table). TLC
  infers role from a profile column the trigger sets at signup; Memika
...
We may eventually want to share code between TLC and Memika — a shared
...
- Onboarding mascot animation pattern (when built) → likely Memika-only
