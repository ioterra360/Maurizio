...
folder, no sections; Plus = unlimited memories, 5 folders, 3 sections;
Pro = everything unlimited plus photos on memories. The caps are enforced
...
- ❌ Not free forever. Freemium (one folder) + Pro via in-app purchase.
...
Three plans — Free / Plus / Pro — sold as **in-app purchases via
...
`app/paywall.tsx` (entitlements `plus` and `pro`). Free = 10 memories and
one folder; Plus and Pro as in `docs/PAYMENTS.md`. No web checkout, ever
