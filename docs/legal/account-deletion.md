> **BOZZA — da far revisionare a un professionista prima della pubblicazione.**
> Questo testo è una proposta redazionale e non costituisce consulenza legale. La versione HTML pronta per ioterra360.github.io/memika-legal/account-deletion/ è `account-deletion.html` (stesso contenuto, senza questa nota). Nota per Google Play: il titolo della pagina deve contenere il nome dell'app ("Memika") e la pagina deve essere raggiungibile senza login.

# Memika — Eliminazione dell'account e dei dati

**Ultimo aggiornamento:** 25 agosto 2026

Questa pagina spiega come eliminare il proprio account **Memika** e i dati associati. Memika è pubblicata da Maurizio Cocco, ditta individuale, Viale Mons. Peri 2, 09079 Tresnuraghes (OR), Italia.

## Opzione 1 — Eliminare l'account dall'App

1. Apri Memika e accedi con il tuo account.
2. Vai in **Impostazioni**.
3. Tocca **Elimina account**.
4. Conferma l'eliminazione nella finestra di richiesta.

L'account viene chiuso immediatamente e non sarà più possibile accedere. I dati vengono eliminati definitivamente entro 30 giorni.

## Opzione 2 — Richiedere l'eliminazione via email

Se non riesci ad accedere all'App (ad esempio l'hai già disinstallata), scrivi a **memikaapp@gmail.com** **dall'indirizzo email con cui ti sei registrato**, con oggetto "Eliminazione account Memika". Non è necessario indicare altro: l'indirizzo del mittente ci serve per verificare che la richiesta provenga dal titolare dell'account.

Elaboriamo la richiesta **entro 30 giorni** dal ricevimento e ti confermiamo via email l'avvenuta eliminazione. Se la richiesta arriva da un indirizzo diverso da quello registrato potremmo chiederti una verifica aggiuntiva.

## Cosa viene eliminato

Con l'eliminazione dell'account vengono cancellati in modo definitivo:

- tutte le **memorie** (termine, lettura, definizione, esempio) e le **cartelle**;
- la **cronologia dei ripassi** e le statistiche di apprendimento;
- il **profilo** (nome visualizzato, preferenze, orari di ripasso);
- l'**identità di accesso** (email e credenziali).

L'eliminazione è **irreversibile**: non è possibile recuperare le memorie dopo la cancellazione. Se vuoi conservare i tuoi contenuti, copiali prima di procedere.

## Cosa può essere conservato

- **Ricevute e cronologia degli acquisti** dell'abbonamento Premium: sono conservate da Apple (App Store) o Google (Google Play) secondo le loro regole; non possiamo eliminarle noi. Eventuali dati contabili obbligatori vengono conservati per i termini previsti dalla legge fiscale.
- **Statistiche aggregate e anonime** (ad esempio il numero complessivo di ripassi effettuati nell'App), che non permettono di identificarti.
- **Copie di backup tecniche**, eliminate automaticamente entro il ciclo di rotazione dei backup (di norma non oltre 30 giorni).
- **Email di assistenza** scambiate con noi, per un massimo di 24 mesi.

## Abbonamento Premium

Eliminare l'account **non disdice automaticamente** l'abbonamento Premium, che è gestito da Apple o Google. Disdicilo prima dalle impostazioni dell'account dello store (iOS: *Impostazioni → [nome] → Abbonamenti*; Android: *Google Play → Pagamenti e abbonamenti → Abbonamenti*) per evitare ulteriori addebiti.

## Domande

Per qualsiasi domanda sui tuoi dati scrivi a **memikaapp@gmail.com**. Maggiori informazioni nell'[Informativa sulla privacy](https://ioterra360.github.io/memika-legal/privacy/).
