...
**Precondizione, da verificare prima di lanciare qualsiasi cosa:** `ls supabase/functions` deve elencare `revenuecat-sync`, la edge function di verifica RevenueCat di B4 (**c'e', da B4**: verificato il 2026-09-04; la frase precedente diceva che la cartella non esisteva ed era scritta prima che B4 la creasse). È il punto 7 della lista "Prima di lanciare". Se manca, fermarsi e chiedere a chi ha spedito B4: senza function il client della vc13 chiama un endpoint inesistente e nessun acquisto scrive mai `profiles.plan`. Stessa cosa per il punto 6: il seed dei tester dentro `20260903100000_plans.sql`, sopra i `create trigger`.
